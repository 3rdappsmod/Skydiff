...
- 문자 모드는 짧은 공통 문자도 그대로 보존합니다. 스마트 모드는 가독성을 위해 변경 사이의 짧은 공통 구간을 함께 강조할 수 있습니다.
- 옵션 변경으로 재비교하는 동안 이전 화면은 유지하지만 탐색은 잠시 비활성화합니다. 실패 시 이전 결과와 통계를 지웁니다.
...
설정·창 위치는 `skydiff-data.json`, 저장된 비교 본문은 `skydiff-comparisons.json`에 분리합니다.
기존 비교 데이터는 시작 시 자동 이전하며, 새 파일에 저장한 뒤에만 기존 항목을 제거합니다.
백업할 때는 사용자 데이터 폴더의 두 파일을 함께 보관하세요. 창 이동·크기 변경은 250ms 지연 저장하고 창을 닫을 때 즉시 반영합니다.
